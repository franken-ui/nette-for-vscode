...
import { LatteFormattingEditProvider } from "./providers/LatteFormattingEditProvider";
import {
    LanguageClient,
...
export function activate(context: vscode.ExtensionContext) {
    ...
    let latteFormatCfg = vscode.workspace.getConfiguration("latte.format");
    if (latteFormatCfg.get<boolean>("enable")) {
        context.subscriptions.push(
            vscode.languages.registerDocumentFormattingEditProvider(
                documentSelector,
                new LatteFormattingEditProvider()
            )
        );
        context.subscriptions.push(
    ...
    vscode.languages.setLanguageConfiguration("latte", {
        indentationRules: {
    ...
    let clientOptions: LanguageClientOptions = {
        documentSelector: ["latte"],
        synchronize: {
            configurationSection: ["latte", "css", "javascript", "emmet"], // the settings to synchronize
        },
    ...
    let client = new LanguageClient(
        "latte",
        localize("latteserver.name", "Latte Language Server"),
        serverOptions,
        clientOptions
    );
